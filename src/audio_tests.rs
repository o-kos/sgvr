...
fn test_sample_reading_table() {
    ...
    for test_case in test_cases {
        ...
        assert_eq!(count, 2, 
            "Read count samples from third position for {} mismatch: expected 2, got {count}", 
            path.display());
        i += 1;
        expected = test_case.samples_0[i];
        ...
        reader.seek(test_case.offset).expect("Failed to seek to second test position");
        let mut samples_1 = vec![0.0f32; 4];
        let count = reader.read(&mut samples_1).expect("Failed to read second position samples");
        assert_eq!(count, 4, 
            "Read count samples from second position for {} mismatch: expected 4, got {count}", 
            path.display());
